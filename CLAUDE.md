...
- **Runtime LLM:** Gemini Flash, **free tier**, via `@google/genai`. Do not use Anthropic or OpenAI at runtime — cost constraint. Confirm the current free Flash model ID in AI Studio rather than hardcoding one from memory.
...
- Set `maxOutputTokens` on every Gemini call. Keep prompts lean.
