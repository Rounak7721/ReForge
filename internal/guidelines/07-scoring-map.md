...
| **Claude/Codex/AI-agent usage** | **20** | `docs/03-prompt-log.md` (5–10 prompts × 4 parts); the AI development process narrative; the video segment on how Claude Code built it; visible process discipline (plan mode, review gate, small commits) |
...
| Debugging/problem solving | **10** | `docs/04-debugging-log.md` with ≥2 full Problem→Prompt→Attempt→Debug→Fix trails |
...
1. **`docs/03-prompt-log.md` and `docs/04-debugging-log.md` are not chores.** They are the second-largest and joint-third-largest scoring items. Writing them as you go is the single highest-leverage habit in this project.
